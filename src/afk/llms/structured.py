...
def json_system_prompt(schema: T) -> str:
    return (
        ...
        f"{schema.model_json_schema(indent=2)}\n"
        "Rules:\n"
...
def parse_and_validate_json(text: str, schema: T) -> T:
    json_str = extract_json_object(text) or text.strip()
    obj = safe_json_loads(json_str)
    if obj is None:
        raise LLMInvalidResponseError(f"Failed to extract valid JSON object from response: {text}")
    try:
        return schema.model_validate(obj)
    except ValidationError as e:
        raise LLMInvalidResponseError(f"JSON does not conform to schema: {e}\nOriginal response: {text}") from e
    

def make_repair_prompt(invalid_response: str, schema: T) -> str:
    return (
