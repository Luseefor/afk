...
description: Behavioral guarantees and edge cases validated by the AFK test suite.
---
...
- This page maps tests to explicit behavior guarantees.
- It serves as a contract for runtime, tooling, memory, and LLM behavior.
- Use it to evaluate regression risk before merging changes.

## When to Use

- You are changing runtime semantics and need contract impact visibility.
- You are debugging a behavior regression.
- You are planning upgrades and need confidence in tested guarantees.
...
- `test_eval_harness_runs_case` (`tests/agents/test_agent_runtime.py:1252`): eval case execution works end-to-end
...
- `test_runner_loads_and_executes_external_mcp_tools` (`tests/agents/test_agent_runtime.py:1347`): runner loads external MCP tools and executes them through standard tool pipeline

## `tests/mcp/test_mcp_store.py`

Covered behaviors:

- `test_mcp_store_materializes_remote_tools_and_executes_call` (`tests/mcp/test_mcp_store.py:14`): external MCP tools are materialized as AFK tools and callable through `ToolRegistry`
- `test_mcp_store_list_tools_uses_cache_until_refresh` (`tests/mcp/test_mcp_store.py:63`): MCP tool discovery cache is reused unless `refresh=True`
- `test_mcp_remote_tool_error_surfaces_as_failed_tool_result` (`tests/mcp/test_mcp_store.py:90`): remote MCP tool errors are surfaced as failed AFK tool results

## `tests/agents/test_prompt_loader.py`

Covered behaviors:

- `test_auto_prompt_loads_for_chat_agent` (`tests/agents/test_prompt_loader.py:29`): auto prompt loads for `ChatAgent` using `CHAT_AGENT.md`
- `test_auto_prompt_loads_for_lowercase_chatagent_name` (`tests/agents/test_prompt_loader.py:38`): lowercase `chatagent` maps to `CHAT_AGENT.md`
- `test_explicit_instruction_file_loads` (`tests/agents/test_prompt_loader.py:47`): explicit `instruction_file` loading works
- `test_inline_instructions_win_over_instruction_file` (`tests/agents/test_prompt_loader.py:61`): inline `instructions` override file prompt source
- `test_prompts_dir_argument_overrides_env` (`tests/agents/test_prompt_loader.py:76`): `prompts_dir` takes precedence over `AFK_AGENT_PROMPTS_DIR`
- `test_env_prompts_dir_used_when_agent_prompts_dir_absent` (`tests/agents/test_prompt_loader.py:92`): env prompt directory is used when no agent override exists
- `test_default_prompts_dir_used_when_no_arg_and_no_env` (`tests/agents/test_prompt_loader.py:105`): default `.agents/prompt` path is used when arg/env are absent
- `test_missing_auto_prompt_file_raises_resolution_error` (`tests/agents/test_prompt_loader.py:118`): missing auto prompt raises configuration failure
- `test_prompt_path_escape_raises_access_error` (`tests/agents/test_prompt_loader.py:124`): prompt path traversal escape is blocked
- `test_prompt_template_renders_context_and_reserved_keys` (`tests/agents/test_prompt_loader.py:140`): template rendering supports flattened + reserved context keys
- `test_prompt_template_missing_variable_raises` (`tests/agents/test_prompt_loader.py:165`): strict undefined variables raise template error
- `test_prompt_store_deduplicates_same_content` (`tests/agents/test_prompt_loader.py:174`): process-level prompt store deduplicates identical prompt text
- `test_prompt_cache_invalidates_when_file_changes` (`tests/agents/test_prompt_loader.py:198`): file stat changes invalidate prompt cache entries
- `test_instruction_roles_append_after_prompt_file` (`tests/agents/test_prompt_loader.py:214`): instruction roles append after file-backed base prompt
...
- adaptive suite mode selection and deterministic parallel output ordering

## `tests/evals/test_budgets_and_assertions.py`

Covered behaviors:

- budget violations mark case as failed
- assertion failures mark case as failed

## `tests/evals/test_reporting_and_datasets.py`

Covered behaviors:

- JSON dataset loader resolves eval cases
- suite JSON report envelope is emitted with schema metadata

## `tests/evals/test_fail_policies.py`

Covered behaviors:

- `fail_fast=True` stops sequential suite on first failure
- `fail_fast=False` runs entire suite even with failures
...
## `tests/llms/test_tool_export.py`

Covered behaviors:

- `test_normalize_json_schema_handles_non_dict_and_missing_fields` (`tests/llms/test_tool_export.py:23`): normalize json schema handles non dict and missing fields
- `test_toolspec_to_openai_tool_maps_core_fields` (`tests/llms/test_tool_export.py:31`): toolspec to openai tool maps core fields
- `test_export_tools_for_provider_supports_aliases` (`tests/llms/test_tool_export.py:41`): export tools for provider supports aliases
- `test_export_tools_for_provider_rejects_unknown_format` (`tests/llms/test_tool_export.py:51`): export tools for provider rejects unknown format
