...
description: Public import rules and extension patterns for stable AFK integration code.
---
...
- Always import from public `afk.*` paths in application code.
- Keep function extensions context-aware, typed, and policy-safe.
- Avoid tight coupling to internal source modules.

## When to Use

- You are migrating code from internal imports to supported public APIs.
- You are improving instructions, policy roles, or tools in existing agent code.
- You want examples that remain stable across AFK releases.
...
@tool(
    ...
    name="search_docs",
    description="Search internal docs for relevant passages and return ranked hits.",
)
...
### 5. Improve system prompts (file-backed + context templates)
...
See [System Prompts](/library/system-prompts) for strict template and cache behavior.
...
- [examples/05_direct_llm_structured_output.py](/library/examples/index#05-direct-llm-structured-output)
- [examples/09_system_prompt_loader.py](/library/examples/index#09-system-prompt-loader)
