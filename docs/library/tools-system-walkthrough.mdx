...
- You are designing custom tools and execution behavior
- You need to understand hook/middleware ordering
- You need safe runtime/skill tool defaults

## Required Capabilities

- typed tool schemas with clear descriptions
- registry policy and middleware configuration
- path/command/network guardrails for side-effect tools

## Decorator to Tool Object Flow
...
## Prebuilt Runtime Filesystem Tools
...
- malformed tool args cause validation failures before execution
- unknown tool names raise registry lookup errors
- policy/sandbox checks can deny execution pre-call
- oversized output can silently truncate if limits are too strict
