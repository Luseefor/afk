...
description: How AFK routes, runs, and observes subagents with deterministic orchestration behavior.
---
...
This topic corresponds to Level 5 in [Agentic System Levels](/library/agentic-levels).
...
- deterministic router logic and stable subagent naming
- explicit context inheritance (`inherit_context_keys`)
- subagent depth/fanout guard configuration
- failure policy choice for subagent errors (`fail`, `degrade`, `continue`)
...
- cyclic routing or excessive depth raises `SubagentRoutingError`
- oversized per-step fanout can exceed configured guardrails
- weak context boundaries can leak unintended parent context
- permissive failure policies can hide critical child-agent failures
