...
<AccordionGroup>
  ...
  <Accordion title="Boundary 2: Tool Runtime" icon="wrench">
    ...
    **Sandbox profiles** restrict what tools can do at the OS level:
    ```python
    @tool(name="run_code", description="Execute code.", sandbox="restricted")
    def run_code(args: CodeArgs) -> dict:
        # Runs in a sandboxed environment with limited filesystem/network access
        ...
    ```
...
AFK supports isolating secrets per tool call, preventing tools from accessing secrets they don't need:

```python
@tool(
    name="query_db",
    description="Query the database.",
    secret_scope=["DB_URL", "DB_PASSWORD"],  # ← Only these env vars are visible
)
