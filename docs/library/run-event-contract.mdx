---
title: Run Events
description: Event schema contract for runner lifecycle events and consumer integration.
---
...
- one dedicated consumer for each `AgentRunHandle.events` stream
- forward-compatible parsing for optional/missing `data` keys
- durable storage/forwarding for terminal lifecycle events
- event routing keyed by `type`, `state`, and `step`
...
- multi-consumer event access violates stream contract
- strict payload assumptions break on optional/extended fields
- dropping terminal events causes incomplete run accounting
- sink-specific write failures can reduce observability coverage
...
1. [Checkpoint & Resume](/library/checkpoint-schema)
2. [API Reference](/library/api-reference)
