...
- Combines:
  ...
  - file-backed prompts (`instruction_file` or auto name) when `instructions is None`
  - each `instruction_role(context, state)` output
- Accepts role output as `str` or `list[str]`.
- Prompt file precedence:
  - inline/callable `instructions` wins
  - else `instruction_file` if provided
  - else auto file from agent name (`UPPER_SNAKE.md`)
- Prompt root precedence:
  - `prompts_dir` argument
  - env `AFK_AGENT_PROMPTS_DIR`
  - `.agents/prompt`
- Prompt templates are rendered with Jinja2 `StrictUndefined`.
- Path escapes from prompt root raise `PromptAccessError`.
...
- [afk/core/runner/api.py](https://github.com/socioy/afk/blob/main/src/afk/core/runner/api.py)
...
- [afk/core/runner/types.py](https://github.com/socioy/afk/blob/main/src/afk/core/runner/types.py)
...
### Provider + Builder API (V2)

Primary entrypoints:

- `create_llm_client(provider=..., settings=...)`
- `LLMBuilder().provider(...).profile(...).build()`
- `register_llm_provider(provider, overwrite=False)`
- `get_llm_provider(provider_id)`
- `list_llm_providers()`

Legacy factory API was removed in llms v2 hard-break.
