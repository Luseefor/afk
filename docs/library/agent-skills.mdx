...
- [afk/agents/types/__init__.py](https://github.com/socioy/afk/blob/main/src/afk/agents/types/__init__.py)
...
- `SKILL.md` frontmatter `name` and `description` are required and define runtime skill identity.
...
- You are enabling operational skills for agent workflows
- You need controlled command execution within skill contexts
- You need runtime auditability for skill reads and commands

## Required Capabilities

- explicit `skills` list and stable `skills_dir` layout
- `SKILL.md` contract for each enabled skill
- command allowlist and execution limits via `SkillToolPolicy`
- monitoring of skill read/command execution records
...
- `skills` values are folder names under `skills_dir` (lookup key only).
...
description: Short runtime-visible description
---
```

- Runtime skill name is read from frontmatter `name` (folder name is ignored after resolution).
- Runtime skill description is read from frontmatter `description`.
...
2. [Security Model](/library/security-model)
